...
class PromptingAgent:
    ...
    def build_prompt(self, query: str, detected_tables: List[str], 
        ...
        try:
            ...
            # Log the complete prompt structure as JSON
            prompt_json = json.dumps(prompt, indent=2)
            logger.info("🤖 LLM PROMPT BUILDER - INPUT JSON:")
            logger.info("=" * 80)
            logger.info(prompt_json)
            logger.info("=" * 80)
    ...
    def log_llm_interaction(self, prompt: str, response: str, success: bool = True, error: Optional[str] = None):
        """Log LLM interaction details for debugging and monitoring"""
        try:
            ...
            if success:
                logger.info("✅ LLM interaction successful")
                ...
                try:
                    ...
                    logger.info("Raw response (not JSON):")
